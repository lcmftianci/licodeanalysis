			...
			if (status == PJ_SUCCESS)
			{
				...
				*/
				...
				if (c)
				{
					...
					int nPos = g_remote_ip.find("t");
					g_remote_ip = g_remote_ip.substr(0, nPos-1);
					LOG(INFO) << "==>>" <<g_remote_ip;
				}
...
int init_video(pj_pool_t *pool)
{
	...
	//mgr->codec_cnt = 0;
	//pjmedia_vid_codec_mgr_set_instance(mgr);
	return 0;
}
...
pj_status_t app_init()
{
   ...
   // app_config.cfg.cb.on_reg_state = &on_reg_state;
	...
	//app_config.cfg.cb.on_media_update = 
...
int sip_call(int acc_id, const char* strCalledNO, const char* strServer, const char* recfile)
{
	if (strServer == NULL)
		return -1;
	char id[80];
	sprintf(id, "sip:%s@%s", strCalledNO, strServer);
...
int send_message(int callid, int accid, char* dest, char* server, const char* buff)
{
	...
	{
		char id[80];
		sprintf(id, "sip:%s@%s", dest, server);
...
void destroy_media()
{
	if (m_bInitMedia)
	{
		if (m_bAttach)
		{
			pjmedia_transport_detach(g_video.transport, &g_video);
			m_bAttach = false;
		}
...
int init_recv_rtp(const char* destip, int destport)
{
	...
	status = pjmedia_transport_attach(g_video.transport, &g_video,
		//&info.rem_addr, 
		&remote_addr,
		NULL, 
...
pj_status_t init_local_rtp()
{
	...
	status = pjmedia_rtp_session_init(&g_video.out_sess, 97, pj_rand());
	status = pjmedia_rtp_session_init(&g_video.in_sess, 97, 0);

	status = pjmedia_transport_udp_create(med_endpt, NULL, g_local_port,  0, &g_video.transport);
...
void send_rtp(char* buff, int len)
{
	...
	if (len > 1400)
	{
		...
		for ( i = 0; i < nBlockNum; i++)
		{
			...
			status = pjmedia_rtp_encode_rtp(&g_video.out_sess,
												97,
												mark,
												poayload_size,
												rtp_ts_len,
			...
			status = pjmedia_transport_send_rtp(g_video.transport, packet, poayload_size + rtphdrlen);
	...
	else
	{
	   ...
	   // vfd->fps.denum = DEFAULT_H264_FPS_DENUM;//1
		...
		int status = pjmedia_rtp_encode_rtp(&g_video.out_sess,
				97,
				1,
				len,
				rtp_ts_len,
		...
		status = pjmedia_transport_send_rtp(g_video.transport, packet, len+rtphdrlen);
	}
