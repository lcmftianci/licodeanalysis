...
/*
  ...
  std::string SdpInfo::stringifyCandidate(const CandidateInfo & candidate) {
    ...
    sdp << "a=candidate:" << candidate.foundation << " " << candidate.componentId << " " << candidate.netProtocol << " " << candidate.priority << " "
      << candidate.hostAddress << " " << candidate.hostPort << " typ " << hostType_str;

	if (m_bsc)
		LIL_LOG(m_cll, this, sdp.str());
