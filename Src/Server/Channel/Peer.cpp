...
cPeer::WebRtcEventListener::WebRtcEventListener(int id, cPeer*a_Participant): id(id), peer(a_Participant)
{
...
cPeer::cPeer(const int &id, cListener *listener): id(id), m_listener(listener)
{
...
bool cPeer::doJoin()
{
	...
	iceConfig.turnServer = "39.106.13.209";
	iceConfig.turnUsername = "along";
	iceConfig.turnPass = "apassword";
	iceConfig.stunServer = "39.106.13.209"; 
...
void cPeer::onJoinedChannel()
{
	...
	m_listener->BroadcastJoinChannel(id);
}
...
{
	m_listener->SendMsgToPeer(id, a_TargetID, a_Msg);
}
