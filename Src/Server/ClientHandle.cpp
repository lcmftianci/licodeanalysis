	...
	m_UniqueID(0)
{
	...
	LOGI("cClientHandle Init OK");

	LOGD("New ClientHandle created at " << this);
}
...
cClientHandle::~cClientHandle()
{
	...
	LOGD("Deleting client:" << GetUsername().c_str() << "at:" << this);
	...
	LOGD("ClientHandle at: " << this << " deleted");
}
...
void cClientHandle::Destroy(void)
{
	...
	// DEBUG:
	LOGD(this <<": client " << "usrname:" << m_Username.c_str()  << " FUNC:" __FUNCTION__);
...
void cClientHandle::SendDisconnect(const int & a_Reason)
{
	...
	if (!m_HasSentDC)
	{
		LOGD("Sending a DC:" <<  StripColorCodes(strReason).c_str());
...
{
	LOGD("An error has occurred on client link for:" << m_Username.c_str() << "@" << m_IPString.c_str() 
		<< "Error Code:" << a_ErrorCode << a_ErrorMsg.c_str()   << "Client disconnected.");
	{
		cCSLock Lock(m_CSOutgoingData);
		m_Link.reset();
	}
