...
class cServerListenCallbacks:public cNetwork::cListenCallbacks
{
	...
	virtual cTCPLink::cCallbacksPtr OnIncomingConnection(const AString & a_RemoteIPAddress, UInt16 a_RemotePort) override{
		LOGI("===>> OnIncomingConnection callback");
...
public:
	cServerListenCallbacks(cServer & a_Server, UInt16 a_Port):
		...
		m_Port(a_Port)
	{
		LOGI("===>> callback cServerListenCallbacks");
	}
};
...
cServer::cTickThread::cTickThread(cServer & a_Server) :
	...
	m_Server(a_Server)
{
	LOGI("===>> Constructor cTickThread");
}

void cServer::cTickThread::Execute(void)
{
	LOGI("===>> function Execute");
...
{
	LOGD("Client:" << a_RemoteIPAddress.c_str()  << " connected!");
